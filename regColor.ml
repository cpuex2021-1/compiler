...
let hikisu_reg =
  S.of_list [ "%x10"; "%x11"; "%x12"; "%x13"; "%f1"; "%f2"; "%f3" ]

let diff_list ls1 ls2 =
  List.fold_left (fun env x -> if List.mem x ls2 then env else x :: env) [] ls1
...
let rec erase_zero ls =
  ...
  | [] -> []
  | (a, b) :: li ->
      if a = Id.izero || a = Id.fzero then erase_zero li
...
let get_tp instr =
  ...
  | Fabs (xt, x) | Fsqrt (xt, x) | Fcvtws (xt, x) | FMovD (xt, x) | FNegD (xt, x)
    ->
  ...
  | Write (xt, a, b, c, d) | Array (xt, a, b, c, d) ->
      erase_zero
        [ xt; (a, Type.Int); (b, Type.Int); (c, Type.Int); (d, Type.Int) ]
  | Farray (xt, a, b, c, d) ->
      erase_zero
        [ xt; (a, Type.Int); (b, Type.Float); (c, Type.Int); (d, Type.Int) ]
  | Read (xt, a, b, c, d, e) ->
      erase_zero
        [
          xt;
...
let adjacent n =
  S.diff (get_adj_list n) (S.union (S.of_list !select_stack) !coalesced_nodes)
    ...
    (fun _ blk ->
      M.iter
        (fun _ instr ->
          ...
          varenv :=
            List.fold_left
              (fun env (x, y) -> M.add x (conv y) env)
              !varenv (get_tp instr))
        blk.instrs)
    fund.blocks;
  List.iter (fun x -> varenv := M.add x Type.Int !varenv) (fund.args @ anyregs);
  List.iter
    (fun x -> varenv := M.add x Type.Float !varenv)
    (fund.fargs @ anyfregs);
        ...
        (fun _ instr ->
          ...
          | Mov (dest, src) ->
              if
                fst dest <> Id.izero
                && fst dest <> Id.fzero
                && src <> Id.izero && src <> Id.fzero
              then
                all_moves :=
                  M.add instr.id (blk.id, instr.id, dest, src) !all_moves
              else ()
          | FMovD (dest, src) ->
              if
                fst dest <> Id.izero
...
let get_K n =
  if get_type n = Type.Float then List.length Asm.allfregs
  else List.length Asm.allregs
...
let node_moves n =
  S.inter (get_move_list n) (S.union !active_moves !worklist_moves)
...
let set_degree n m = degree := M.add m n (M.remove m !degree)
...
let conservative nodes =
  let k = ref 0 in
  S.iter (fun n -> if get_degree n >= get_K n then k := !k + 1) nodes;
  S.is_empty nodes || !k < get_K (S.min_elt nodes)

let rec get_alias n =
  if S.mem n !coalesced_nodes then get_alias (M.find n !alias) else n
...
let freeze_moves u =
  S.iter
    (fun m ->
      ...
      if
        (not (S.mem v !precolored))
        && S.is_empty (node_moves v)
...
let insert_save_bunki fund block instr x new_temp =
  ...
  let target_instr =
    if M.is_empty target_blk.instrs then None
    else Some (M.find target_blk.head target_blk.instrs)
  in
  let id = Block.gen_instr_id () in
  new_temps := S.add new_temp !new_temps;
  let new_instr =
  ...
  if instr.nextId = "" then
    ...
    let new_instr =
      create_instr id block.id
        ...
        instr.id instr.nextId S.empty S.empty
    in
    instr.nextId <- id;
    (if new_instr.nextId = "" then block.tail <- id
    else
      let next = M.find new_instr.nextId block.instrs in
      next.predId <- id);
    block.instrs <- M.add id new_instr block.instrs;
...
let initialize is_first fundef =
  ...
  if is_first then (
    ...
    initial :=
      M.fold
        (fun x t env ->
          if (not (S.mem x !precolored)) && t <> Type.Unit then S.add x env
          else env)
        !varenv S.empty;
  ...
  M.iter
    (fun _ block ->
      ...
      let rec iter instr_id =
        let liveout = !live in
        let instr =
          if instr_id = "" then None else Some (M.find instr_id block.instrs)
        in
        let df, us =
          if instr_id = "" then (S.empty, S.of_list (fund.args @ fund.fargs))
          else
            (fun (a, b) -> (S.of_list a, S.of_list b))
              (Block.def_use (rm_some instr))
        in
...
let select_simplify_node fund =
  let priority0 =
    S.empty
    (*S.inter (S.of_list (fund.args @ fund.fargs)) !simplify_worklist*)
  in
  let priority1 = S.inter !arg_nodes !simplify_worklist in
  let priority2 = S.inter !ret_nodes !simplify_worklist in
  let priority3 =
    S.diff !simplify_worklist (S.union priority0 (S.union priority1 priority2))
  in
  if S.is_empty priority3 then
    if S.is_empty priority2 then
      if S.is_empty priority1 then
        if S.is_empty priority0 then assert false else S.min_elt priority0
...
let coalesce fund =
  ...
  if u = v then (
    ...
    || (not (S.mem u !precolored))
       && conservative (S.union (adjacent u) (adjacent v))
  then (
    coalesced_moves := S.add m !coalesced_moves;
    combine u v;
    add_worklist u)
  else active_moves := S.add m !active_moves
...
let select_spill fundef =
  let m =
    if !spill_cnt mod 2 = 1 then S.min_elt !spill_worklist
        ...
        (fun instr ->
          ...
          | CallDir ((dest, _), Id.L name, args, fargs)
            when Id.L name <> fund.name ->
              ...
              Format.eprintf "\n";
              let uses =
                S.union (Asm.get_use_regs name) (S.of_list args_called)
              in
              let uses_list = S.fold (fun a li -> a :: li) uses [] in
              S.iter
                (fun n -> set_not_together n uses_list)
                (S.inter instr.liveIn instr.liveOut);
              List.iter2
                (fun call called ->
                  set_together call [ called ];
                  set_not_together call
                    (S.fold
                       (fun x e -> x :: e)
                       (S.remove called (S.of_list args_called))
                       []))
...
let rec s_dif s li =
  match li with [] -> s | l :: ls -> s_dif (S.remove l s) ls
...
let select_color fund n ok_colors =
  ...
  if M.mem n !together then (
    ...
    if n = "num.6178.7794.13799" then
      let z = S.fold (fun x e -> x :: e) ok_colors [] in
      Asm.print_regs z
    else ();
    List.iter
      (fun a ->
        let a = if M.mem a !color then M.find a !color else a in
        if S.mem a ok_colors then (
          ans := a;
          ( (*Format.eprintf "ok_colour %s\n" a*) ))
        else ())
      (M.find n !together))
  else if M.mem n !no_together then
    let no = M.find n !no_together in
    let no =
      List.map (fun a -> if M.mem a !color then M.find a !color else a) no
    in
...
let assign_colors fundef =
  while !select_stack <> [] do
    ...
    let ok_colors =
      ref
        (S.of_list
           (if get_type n = Type.Float then Asm.allfregs else Asm.allregs))
    in
    S.iter
      (fun w ->
  ...
  S.iter
    (fun n -> color := M.add n (get_color (get_alias n)) !color)
    !coalesced_nodes
...
let rec main is_first (fund : Block.fundef) =
  ...
  if not (S.is_empty !spilled_nodes) then (
    ...
    (try
       ...
       let args =
         List.map
           (fun x -> try M.find x !color with Not_found -> "%x0")
